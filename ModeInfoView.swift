...
struct ModeInfoView: View {
    ...
    private var infoData:
        (
            title: String, subtitle: String, ratio: String, detail: String,
        ...
        )
    {
        ...
        case .calm:
            return (
                ...
                detail:
                    "A practice of absolute balance. By making the Breathe In, the pause, and the release perfectly equal, we anchor ourselves in a state of unwavering calm.",
                specialFeature:
                    "During the release phase, the device will emit a continuous, deep vibration—mimicking the 'Humming Bee Breath' to sustain your focus."
            )
        case .focus:
            return (
                title: "Dharana — धारणा",
                ...
                detail:
                    "A practice that balances the mind and brings steady attention through the breath.",
                specialFeature:
                    "Follow the voice guidance or on-screen text to alternately block your nostrils as the cycle progresses."
            )
        case .sleep:
            return (
                title: "Nidra — निद्रा",
                ...
                detail:
                    "An extended calming rhythm. A natural tranquilizer for the body, signaling that it is safe to surrender to rest.",
                specialFeature:
                    "During the release phase, the device will emit a continuous, deep vibration—mimicking the 'Humming Bee Breath' to sustain your calm."
            )
        }
    }
    ...
    private var dharanaDiagram: some View {
        VStack(spacing: 16) {
            Canvas { ctx, size in
                ...
                ctx.stroke(
                    rightPath,
                    with: .color(BreathingMode.calm.color.opacity(0.7)),
                    style: StrokeStyle(lineWidth: 3, lineCap: .round)
                )
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 24) {
                ...
                Canvas { ctx, size in
                    ...
                    ctx.stroke(
                        path,
                        with: .color(BreathingMode.calm.color.opacity(0.8)),
                        style: StrokeStyle(
                            lineWidth: 4,
                            lineCap: .round,
                            lineJoin: .round
    ...
    private var nidraDiagram: some View {
        VStack(spacing: 16) {
            Canvas { ctx, size in
                ...
                let p1 = CGPoint(
                    x: startX + ((4 / totalTime) * (endX - startX)),
                    y: 10
                )
                let p2 = CGPoint(
                    x: startX + ((11 / totalTime) * (endX - startX)),
                    y: 10
                )
